...
impl VaultContract {
    ...
    fn update_rewards(env: &Env, user: &Address) {
        ...
        let last_updated = env.storage().persistent().get::<DataKey, u64>(&DataKey::UserLastUpdated(user.clone())).unwrap_or(env.ledger().timestamp());
        let current_time = env.ledger().timestamp();
        
        let elapsed = (current_time - last_updated) as i128;
        let rate = env.storage().instance().get::<DataKey, i128>(&DataKey::RewardRate).unwrap_or(0);
        ...
        env.storage().persistent().set(&DataKey::UserPendingRewards(user.clone()), &(existing_rewards + new_rewards));
        env.storage().persistent().set(&DataKey::UserLastUpdated(user.clone()), &current_time);
    }
    ...
    pub fn deposit(env: Env, user: Address, amount: i128) {
        ...
        let token = env.storage().instance().get::<DataKey, Address>(&DataKey::Token).unwrap();
        ...
        balance += amount;
        env.storage().persistent().set(&DataKey::UserBalance(user.clone()), &balance);
    ...
    pub fn claim_rewards(env: Env, user: Address) {
        ...
        env.storage().persistent().set(&DataKey::UserPendingRewards(user.clone()), &0i128);
        ...
        env.events().publish((symbol_short!("CLAIM"), user.clone()), pending_rewards);
    }

    pub fn get_balance(env: Env, user: Address) -> i128 {
        env.storage().persistent().get::<DataKey, i128>(&DataKey::UserBalance(user)).unwrap_or(0)
    }

    pub fn get_pending_rewards(env: Env, user: Address) -> i128 {
        env.storage().persistent().get::<DataKey, i128>(&DataKey::UserPendingRewards(user)).unwrap_or(0)
    }
}
